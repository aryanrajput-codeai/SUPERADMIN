...
function NavItem({ id, label, icon: Icon, isActive, isExpanded, onClick }: NavItemProps) {
  ...
  return (
    <button
      ...
      className={`w-full flex items-center rounded-xl text-sm font-medium transition-all relative group h-11 cursor-pointer focus:outline-hidden focus:ring-2 focus:ring-indigo-500/40 focus:ring-offset-2 focus:ring-offset-slate-900 ${
        isExpanded 
...
export default function Sidebar({ 
  ...
  return (
    <aside 
      className={`fixed inset-y-0 left-0 z-40 flex flex-col h-screen bg-[#0f172a] text-slate-300 border-r border-slate-800 transition-[width,transform] duration-200 ease-in-out ${
        isMobileOpen ? 'translate-x-0 shadow-[5px_0_30px_rgba(0,0,0,0.4)]' : '-translate-x-full'
      } lg:translate-x-0 lg:static lg:flex ${
        isExpanded ? 'w-[260px]' : 'lg:w-[72px] w-[260px]'
      }`}
    >
      {/* Brand Header */}
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-950/30 flex flex-col gap-4 shrink-0">
        ...
        <button
          ...
          onClick={onLogout}
          className={`w-full flex items-center rounded-xl text-sm font-medium text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 transition-all cursor-pointer h-11 focus:outline-hidden focus:ring-2 focus:ring-rose-500/40 ${
            isExpanded ? 'px-3.5 py-2.5 justify-start space-x-3' : 'lg:justify-center lg:space-x-0 lg:px-0 px-3.5 py-2.5 justify-start space-x-3'
          }`}
